...
osx-build: # Create the executable for OSX
	@GOOS=darwin GOARCH=amd64 go build -o comics-downloader-osx ./cmd/

windows-build: # Create the executable for WINDOWS
	@GOOS=windows GOARCH=amd64 go build -o comics-downloader.exe ./cmd/

linux-build: # Create the executable for LINUX
	@GOOS=linux GOARCH=amd64 go build -o comics-downloader ./cmd/
